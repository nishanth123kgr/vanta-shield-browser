...
async function loadRules() {
    try {
        ...
        const response = await fetch('./rules.yar');
        if (!response.ok) {
        ...
        // Cache the rules
        compiledRulesCache = rulesText;
        console.log(`Loaded ${rulesText.split('\n').length} lines of YARA rules`);
...
async function analyzeYara(file) {
    try {
        ...
        console.log(`Starting YARA analysis of file: ${file.name} (${file.size} bytes)`);
...
// Scan raw binary data instead of a file object
async function scanBinaryData(binaryData, fileName = 'unknown') {
    try {
        if (!binaryData || binaryData.length === 0) {
            throw new Error('No binary data provided for analysis');
        }

        // Initialize YARA if not already done
        await initializeYara();

        console.log(`Starting YARA analysis of binary data: ${fileName} (${binaryData.length} bytes)`);
