...
chrome.downloads.onCreated.addListener((downloadItem) => {
  ...
  // Scan with Yara if the download is a file
  if (downloadItem.filename && downloadItem.state === 'in_progress') {
    console.log(`Scanning download: ${downloadItem.filename}`);

    initializeYara().then(() => {
      console.log('YARA engine initialized for download scan');
    }).catch(error => {
      console.error('Failed to initialize YARA for download scan:', error);
    });
      
    
    // Here you would integrate with your YARA scanning logic
    // For example:
    // scanDownloadedFile(downloadItem.filename).then(result => {
    //   if (result.isThreat) {
    //     console.warn(`Threat detected in downloaded file: ${downloadItem.filename}`);
  ...
  try {
      ...
      if (isThreat) {
        ...
        chrome.tabs.update(tabId, {
          url: chrome.runtime.getURL("./pages/index-standalone.html") + "?domain=" + encodeURIComponent(hostName)
        });
      }
    } catch (error) {
